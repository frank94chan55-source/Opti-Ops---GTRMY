...
export default function App() {
  ...
  const [tonnageAdjustment, setTonnageAdjustment] = useState(0);
  const [isSimulating, setIsSimulating] = useState(false);
  const [loadingFactor, setLoadingFactor] = useState(1.1);
  const [fteCost, setFteCost] = useState(4500);
  const [scCost, setScCost] = useState(3000);

  const currentFte = useMemo(() => currentHC.t1.fte + currentHC.t2.fte, [currentHC]);
  const currentSc = useMemo(() => currentHC.t1.sc + currentHC.t2.sc, [currentHC]);
  ...
  const totalTonnage = useMemo(() => tonnage.t1 + tonnage.t2, [tonnage]);
  
  const overallProductivity = useMemo(() => {
    const hc1 = currentHC.t1.fte + currentHC.t1.sc;
    const hc2 = currentHC.t2.fte + currentHC.t2.sc;
    const totalHC = hc1 + hc2;
    if (totalHC === 0) return Math.round((productivity.t1 + productivity.t2) / 2);
    return Math.round((productivity.t1 * hc1 + productivity.t2 * hc2) / totalHC);
  }, [productivity, currentHC]);
  ...
  const handleTotalTonnageChange = (newTotal: number) => {
    ...
    setTonnage({
      t1: Math.round(tonnage.t1 * ratio),
      t2: Math.round(tonnage.t2 * ratio)
    });
  };
  ...
  const handleOverallProductivityChange = (newOverall: number) => {
    ...
    if (totalHC === 0) {
      const half = Math.round(newOverall / 2);
      setProductivity({
        t1: half,
        t2: newOverall - half
      });
    ...
    const t1 = Math.round((hc1 / totalHC) * newOverall * 2);
    const t2 = Math.round((hc2 / totalHC) * newOverall * 2);
    setProductivity({ t1, t2 });
  };

  // Keep productivity targets in sync with headcount ratios when headcount changes
  useEffect(() => {
  ...
  const adjustedTonnage = useMemo(() => {
    const factor = isSimulating ? (1 + (tonnageAdjustment / 100)) : 1;
    return {
      t1: tonnage.t1 * factor,
      t2: tonnage.t2 * factor
    };
  }, [tonnage, tonnageAdjustment, isSimulating]);

  const manpower = useMemo(() => {
    const t1HoursRaw = calculateHours(adjustedTonnage.t1, productivity.t1) * loadingFactor;
    const t2HoursRaw = calculateHours(adjustedTonnage.t2, productivity.t2) * loadingFactor;
    
    const totalHours = t1HoursRaw + t2HoursRaw;
    ...
    const t1Fte = Math.ceil(globalFte * (currentFte > 0 ? currentHC.t1.fte / currentFte : 1/2));
    const t2Fte = Math.ceil(globalFte * (currentFte > 0 ? currentHC.t2.fte / currentFte : 1/2));

    const t1Sc = Math.ceil(globalSc * (currentSc > 0 ? currentHC.t1.sc / currentSc : 1/2));
    const t2Sc = Math.ceil(globalSc * (currentSc > 0 ? currentHC.t2.sc / currentSc : 1/2));

    // 3. Sum up the rounded station values for the final totals
    const totalFte = t1Fte + t2Fte;
    const totalSc = t1Sc + t2Sc;
    ...
    return {
      ...
      t1HC: t1Fte + t1Sc,
      t2HC: t2Fte + t2Sc,
      t1Hours: (t1Fte + t1Sc) * MONTHLY_HOURS,
      t2Hours: (t2Fte + t2Sc) * MONTHLY_HOURS,
      monthlyHours: MONTHLY_HOURS,
      adjustedTotalTonnage: adjustedTonnage.t1 + adjustedTonnage.t2
    };
  }, [adjustedTonnage, productivity, loadingFactor, currentHC, currentFte, currentSc, effectiveMinFtePercent, shiftHours]);
  ...
  const chartData = [
    { name: 'Import', heads: manpower.t1HC, color: '#18181b' },
    { name: 'Export', heads: manpower.t2HC, color: '#3f3f46' },
    { name: 'KUL Station', heads: manpower.totalHC, color: '#ef2536' },
  ];
          ...
          <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <div className="text-right">
                ...
                <p className="text-sm font-mono text-zinc-950">GTRMY</p>
              </div>
            </div>
          </div>
      </header>
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <section className="bg-white border border-zinc-200 rounded-2xl shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-zinc-100 bg-zinc-50/50 flex items-center gap-2">
              ...
              <h2 className="text-base font-mono font-bold uppercase tracking-widest text-brand-red">Monthly Tonnage Configuration</h2>
            </div>
            
            <InputGroup 
              label="KUL Total Tonnage" 
              value={totalTonnage} 
            ...
            <div className="px-5 py-2.5 bg-zinc-50/50 border-b border-zinc-100">
              <p className="text-[10px] font-mono text-zinc-600 uppercase tracking-widest">Process Breakdown</p>
            </div>

            <InputGroup 
              label="Import Tonnage" 
              value={tonnage.t1} 
              onChange={(v) => setTonnage(prev => ({ ...prev, t1: v }))}
              min={0} 
              max={totalTonnage} 
              unit="TONS"
            />
            <InputGroup 
              label="Export Tonnage" 
              value={tonnage.t2} 
              onChange={(v) => setTonnage(prev => ({ ...prev, t2: v }))}
              min={0} 
              max={totalTonnage} 
              unit="TONS"
            />

            <div className="px-5 py-4 border-t border-b border-zinc-100 bg-amber-50/30">
              <div className="flex items-center justify-between mb-4">
                <label className="text-[10px] font-mono font-bold uppercase tracking-widest text-amber-700 flex items-center gap-2">
                  <TrendingUp size={12} />
                  Simulation Mode
                </label>
                <button 
                  ...
                  className={cn(
                    "relative inline-flex h-5 w-10 shrink-0 cursor-pointer items-center rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-950 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50",
                    isSimulating ? "bg-amber-600" : "bg-zinc-200"
                  )}
                >
                  <span className={cn(
                    "pointer-events-none block h-4 w-4 rounded-full bg-white shadow-lg ring-0 transition-transform",
                    isSimulating ? "translate-x-5" : "translate-x-1"
                  )} />
                </button>
              </div>
              ...
              {isSimulating && (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-mono uppercase text-zinc-500">Volume Delta</span>
                    <span className={cn(
                      "text-xs font-mono font-bold",
                      tonnageAdjustment > 0 ? "text-emerald-700" : tonnageAdjustment < 0 ? "text-rose-700" : "text-zinc-500"
                    )}>
                      {tonnageAdjustment > 0 ? `+${tonnageAdjustment}` : tonnageAdjustment}%
                    </span>
                  </div>
                  <input 
                    type="range" 
                    ...
                    max="100" 
                    step="1" 
                    value={tonnageAdjustment} 
                    onChange={(e) => setTonnageAdjustment(Number(e.target.value))}
                  ...
                  <div className="pt-2 border-t border-amber-100/50">
                    <div className="flex justify-between items-center text-[9px] font-mono">
                      <span className="text-zinc-500">Adjusted Total:</span>
                      <span className="font-bold text-amber-700">
                        {Math.round(manpower.adjustedTotalTonnage).toLocaleString()} TONS
                      </span>
                    </div>
                  </div>
                </motion.div>
          ...
          <section className="bg-white border border-zinc-200 rounded-2xl shadow-sm overflow-hidden">
            ...
            <InputGroup 
              label="KUL Overall Productivity" 
            ...
            <InputGroup 
              label="Import Productivity" 
              value={productivity.t1} 
              onChange={(v) => setProductivity(prev => ({ ...prev, t1: v }))}
              min={1} 
              max={overallProductivity} 
              unit="KG/HR"
            />
            <InputGroup 
              label="Export Productivity" 
              value={productivity.t2} 
              onChange={(v) => setProductivity(prev => ({ ...prev, t2: v }))}
              min={1} 
              max={overallProductivity} 
              unit="KG/HR"
            />
          ...
          <section className="bg-white border border-zinc-200 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="p-4 bg-zinc-50/30">
              <div className="flex items-center gap-2 mb-4 px-1">
                ...
                <h3 className="text-xs font-mono font-bold uppercase tracking-widest text-zinc-900">Headcount Costs (MYR)</h3>
              </div>
              <div className="space-y-4 p-3 bg-zinc-50 rounded-lg border border-zinc-100">
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-white border border-zinc-300 rounded pl-10 pr-2 py-1.5 text-xs font-mono text-zinc-900 focus:outline-none focus:border-zinc-500"
                    />
                  </div>
                  <p className="text-[9px] font-mono text-zinc-400 text-right mt-1">
                    (SGD {(fteCost * 0.32).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })})
                  </p>
                </div>
                <div className="space-y-1">
                  ...
                  <p className="text-[9px] font-mono text-zinc-400 text-right mt-1">
                    (SGD {(scCost * 0.32).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })})
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard 
              ...
              subtext={isSimulating 
                ? `${Math.round(manpower.totalHours).toLocaleString()} Hrs @ ${Math.round(manpower.adjustedTotalTonnage).toLocaleString()} Tons`
                : `${Math.round(manpower.totalHours).toLocaleString()} Productive Hours`
              }
              icon={Users}
              trend={isSimulating ? "Simulated" : undefined}
            />
            <StatCard 
            ...
            <StatCard 
              ...
              value={`RM ${costMovement.toLocaleString()}  (SGD ${(costMovement * 0.32).toLocaleString()})`} 
          ...
          <div className="grid grid-cols-1 gap-8">
            <section className="bg-white border border-zinc-200 rounded-2xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-base font-mono font-bold uppercase tracking-widest text-brand-red">Recommended Headcount Breakdown by Process</h2>
              ...
              <table className="w-full text-left font-mono text-sm">
                <thead>
                  <tr className="border-b border-zinc-100 text-zinc-600 uppercase tracking-tighter">
                    <th className="px-6 py-5 font-medium">Process</th>
                  ...
                  {(() => {
                    ...
                    return [
                      { 
                        name: 'Import', 
                        hours: shiftHours, 
                        reqHours: manpower.t1Hours, 
                        currentHC: currentHC.t1.fte + currentHC.t1.sc 
                      },
                      { 
                        name: 'Export', 
                        hours: shiftHours, 
                        reqHours: manpower.t2Hours, 
                        currentHC: currentHC.t2.fte + currentHC.t2.sc 
                      },
                      { 
                        name: 'KUL Total', 
                        hours: '-', 
                  ...
                  {(() => {
                    const stationData = [
                      { 
                        id: 't1', 
                        name: 'Import', 
                        recFte: manpower.t1Fte, 
                        recSc: manpower.t1Sc, 
                        currentFte: currentHC.t1.fte, 
                        currentSc: currentHC.t1.sc 
                      },
                      { 
                        id: 't2', 
                        name: 'Export', 
                        recFte: manpower.t2Fte, 
                        recSc: manpower.t2Sc, 
                        currentFte: currentHC.t2.fte, 
                        currentSc: currentHC.t2.sc 
                      },
                    ];
                    ...
                    return allRows.map((row, i) => {
                      const vPercent = row.name === 'KUL Station' 
                      ...
                      return (
                        <tr key={i} className={cn("hover:bg-zinc-50/50 transition-colors", row.highlight && "bg-zinc-50 font-bold")}>
                          ...
                          <td className={cn(
                            ...
                            {`RM ${(row.rowSavings || 0).toLocaleString()}  (SGD ${((row.rowSavings || 0) * 0.32).toLocaleString()})`}
                          </td>
